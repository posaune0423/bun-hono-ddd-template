...
describe("executeCreatePost", () => {
  ...
  describe("successful creation", () => {
    it("should create a post with valid input", async () => {
      const result = await executeCreatePost(
        { postRepository: ctx.postRepository, userRepository: ctx.userRepository },
        { title: "Test Post", content: "This is test content.", authorId: testUser.id },
      );
    ...
    it("should create multiple posts for the same author", async () => {
      const result1 = await executeCreatePost(
        { postRepository: ctx.postRepository, userRepository: ctx.userRepository },
        { title: "First Post", content: "First content.", authorId: testUser.id },
      );

      const result2 = await executeCreatePost(
        { postRepository: ctx.postRepository, userRepository: ctx.userRepository },
        { title: "Second Post", content: "Second content.", authorId: testUser.id },
      );
  ...
  describe("not found errors", () => {
    it("should return NotFoundError when author does not exist", async () => {
      const result = await executeCreatePost(
        { postRepository: ctx.postRepository, userRepository: ctx.userRepository },
        { title: "Orphan Post", content: "Content without author.", authorId: "non-existent-user-id" },
      );
  ...
  describe("post persistence", () => {
    it("should persist the created post", async () => {
      const createResult = await executeCreatePost(
        { postRepository: ctx.postRepository, userRepository: ctx.userRepository },
        { title: "Persisted Post", content: "This should be saved.", authorId: testUser.id },
      );

      expect(createResult.isOk()).toBe(true);

      if (createResult.isOk()) {
        const findResult = await ctx.postRepository.findById(createResult.value.post.id);
